...
CANNON.Demo = function(options){
    ...
    function peerDataSend(){
      ...
      if ((connectionExists(conn1) || (connectionExists(conn2))) && (curTime - prevTimeEmit > 25)){
        ...
        //   sphereY: sphereMesh.position.y,
        //   sphereZ: sphereMesh.position.z
        // };
        var positionX = [];
        var positionY = [];
        var positionZ = [];
        ...
        for(var i=0; i<N; i++){
            var b = bodies[i];
            positionX.push(b.position.x);
            positionY.push(b.position.y);
            positionZ.push(b.position.z);
            if(b.quaternion){
              quaternionIdx.push(i);
              quaternionX.push(b.quaternion.x);
              quaternionY.push(b.quaternion.y);
              quaternionZ.push(b.quaternion.z);
              quaternionW.push(b.quaternion.w);
            }
        }
        var data = {
          posX: positionX,
